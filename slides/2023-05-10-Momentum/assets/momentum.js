...
function renderTaxonomy(div) {
  ...
  var top = 270
  var spacing = 190
  var updateDR = genPhase(0,top,0, [-1.4,1.4], "Ripples", "<small>R's eigenvalues are complex, and the iterates display low frequency ripples. Surprisingly, the convergence rate " + MathCache("two-sqrt-beta") + " is independent of " + MathCache("alpha") + " and " + MathCache("lambda-i") + ".</small>")
  updateDR(getTrace(0.0017,0.94,1,1))

  var updateMC = genPhase(1,top,spacing*1,[-1.4,1.4], "Monotonic Decrease", "<small>R's eigenvalues are both real, are positive, and have norm less than one. The behavior here resembles gradient descent.</small>")
  updateMC(getTrace(0.00093, 0.16,1,1))

  var updateD = genPhase(2,top,spacing*2,[-1.4,1.4], "1-Step Convergence", "<small>When " + MathCache("alpha-equals-one-over-lambda-i") + ", and " + MathCache("beta-equals-zero") + ", we converge in one step. This is a very special point, and kills the error in the eigenspace completely.</small>")
  updateD(getTrace(0.0213, 0.06,1,1))

  var updateIC = genPhase(3,top,spacing*3,[-1.4,1.4], "Monotonic Oscillations", "<small>When " + MathCache("alpha-gt-one-over-lambda-i") + ", the iterates flip between " + MathCache("plus") + " and " + MathCache("minus") + " at each iteration. These are often referred to as 'oscillations' in gradient descent.</small>")
  updateIC(getTrace(0.01, 0.0001,1,1))

  var updateMO = genPhase(4,top,spacing*4,[-5,5], "Divergence", "<small>When " + 
  MathCache("max-sigma-one")
  // "max{|σ<sub>1</sub>,σ<sub>2</sub>|}"
  + ", the iterates diverge. </small>")
